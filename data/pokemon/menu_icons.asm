; party menu icons

MonMenuIcons:
	table_width 1
	db ICON_BULBIZARRE   ; BULBIZARRE
	db ICON_BULBIZARRE   ; HERBIZARRE
	db ICON_BULBIZARRE   ; FLORIZARRE
	db ICON_SALAMECHE  ; SALAMECHE
	db ICON_SALAMECHE  ; REPTINCEL
	db ICON_BIGMON      ; DRACAUFEU
	db ICON_CARAPUCE    ; CARAPUCE
	db ICON_CARAPUCE    ; CARABAFFE
	db ICON_CARAPUCE    ; TORTANK
	db ICON_CATERPILLAR ; CHENIPAN
	db ICON_CATERPILLAR ; CHRYSACIER
	db ICON_MOTH        ; PAPILUSION
	db ICON_CATERPILLAR ; ASPICOT
	db ICON_CATERPILLAR ; COCONFORT
	db ICON_BUG         ; DARDARGNAN
	db ICON_BIRD        ; ROUCOOL
	db ICON_BIRD        ; ROUCOUPS
	db ICON_BIRD        ; ROUCARNAGE
	db ICON_FOX         ; RATTATA
	db ICON_FOX         ; RATTATAC
	db ICON_BIRD        ; PIAFABEC
	db ICON_BIRD        ; RAPASDEPIC
	db ICON_SERPENT     ; ABO
	db ICON_SERPENT     ; ARBOK
	db ICON_PIKACHU     ; PIKACHU
	db ICON_PIKACHU     ; RAICHU
	db ICON_MONSTER     ; SABELETTE
	db ICON_MONSTER     ; SABLAIREAU
	db ICON_FOX         ; NIDORAN_F
	db ICON_FOX         ; NIDORINA
	db ICON_MONSTER     ; NIDOQUEEN
	db ICON_FOX         ; NIDORAN_M
	db ICON_FOX         ; NIDORINO
	db ICON_MONSTER     ; NIDOKING
	db ICON_MELOFEE    ; MELOFEE
	db ICON_MELOFEE    ; MELODELFE
	db ICON_FOX         ; GOUPIX
	db ICON_FOX         ; FEUNARD
	db ICON_RONDOUDOU  ; RONDOUDOU
	db ICON_RONDOUDOU  ; GRODOUDOU
	db ICON_BAT         ; NOSFERAPTI
	db ICON_BAT         ; NOSFERALTO
	db ICON_MYSTHERBE      ; MYSTHERBE
	db ICON_MYSTHERBE      ; ORTIDE
	db ICON_MYSTHERBE      ; RAFFLESIA
	db ICON_BUG         ; PARAS
	db ICON_BUG         ; PARASECT
	db ICON_CATERPILLAR ; MIMITOSS
	db ICON_MOTH        ; AEROMITE
	db ICON_TAUPIQUEUR     ; TAUPIQUEUR
	db ICON_TAUPIQUEUR     ; TRIOPIKEUR
	db ICON_FOX         ; MIAOUSS
	db ICON_FOX         ; PERSIAN
	db ICON_MONSTER     ; PSYKOKWAK
	db ICON_MONSTER     ; AKWAKWAK
	db ICON_FIGHTER     ; FEROSINGE
	db ICON_FIGHTER     ; COLOSSINGE
	db ICON_FOX         ; CANINOS
	db ICON_FOX         ; ARCANIN
	db ICON_PTITARD     ; PTITARD
	db ICON_PTITARD     ; TETARTE
	db ICON_PTITARD     ; TARTARD
	db ICON_HUMANSHAPE  ; ABRA
	db ICON_HUMANSHAPE  ; KADABRA
	db ICON_HUMANSHAPE  ; ALAKAZAM
	db ICON_FIGHTER     ; MACHOC
	db ICON_FIGHTER     ; MACHOPEUR
	db ICON_FIGHTER     ; MACKOGNEUR
	db ICON_MYSTHERBE      ; CHETIFLOR
	db ICON_MYSTHERBE      ; BOUSTIFLOR
	db ICON_MYSTHERBE      ; EMPIFLOR
	db ICON_JELLYFISH   ; TENTACOOL
	db ICON_JELLYFISH   ; TENTACRUEL
	db ICON_RACAILLOU     ; RACAILLOU
	db ICON_RACAILLOU     ; GRAVALANCH
	db ICON_RACAILLOU     ; GROLEM
	db ICON_EQUINE      ; PONYTA
	db ICON_EQUINE      ; GALOPA
	db ICON_RAMOLOSS    ; RAMOLOSS
	db ICON_RAMOLOSS    ; FLAGADOSS
	db ICON_VOLTORBE     ; MAGNETI
	db ICON_VOLTORBE     ; MAGNETON
	db ICON_BIRD        ; CANARTICHO
	db ICON_BIRD        ; DODUO
	db ICON_BIRD        ; DODRIO
	db ICON_LOKHLASS      ; OTARIA
	db ICON_LOKHLASS      ; LAMANTINE
	db ICON_BLOB        ; TADMORV
	db ICON_BLOB        ; GROTADMORV
	db ICON_SHELL       ; KOKIYAS
	db ICON_SHELL       ; CRUSTABRI
	db ICON_GHOST       ; FANTOMINUS
	db ICON_GHOST       ; SPECTRUM
	db ICON_GHOST       ; ECTOPLASMA
	db ICON_SERPENT     ; ONIX
	db ICON_HUMANSHAPE  ; SOPORIFIK
	db ICON_HUMANSHAPE  ; HYPNOMADE
	db ICON_SHELL       ; KRABBY
	db ICON_SHELL       ; KRABBOSS
	db ICON_VOLTORBE     ; VOLTORBE
	db ICON_VOLTORBE     ; ELECTRODE
	db ICON_MYSTHERBE      ; NOEUNOEUF
	db ICON_MYSTHERBE      ; NOADKOKO
	db ICON_MONSTER     ; OSSELAIT
	db ICON_MONSTER     ; OSSATUEUR
	db ICON_FIGHTER     ; KICKLEE
	db ICON_FIGHTER     ; TYGNON
	db ICON_MONSTER     ; EXCELANGUE
	db ICON_BLOB        ; SMOGO
	db ICON_BLOB        ; SMOGOGO
	db ICON_EQUINE      ; RHINOCORNE
	db ICON_MONSTER     ; RHINOFEROS
	db ICON_MELOFEE    ; LEVEINARD
	db ICON_MYSTHERBE      ; SAQUEDENEU
	db ICON_MONSTER     ; KANGOUREX
	db ICON_FISH        ; HYPOTREMPE
	db ICON_FISH        ; HYPOCEAN
	db ICON_FISH        ; POISSIRENE
	db ICON_FISH        ; POISSOROY
	db ICON_STARI      ; STARI
	db ICON_STARI      ; STAROSS
	db ICON_HUMANSHAPE  ; M__MIME
	db ICON_BUG         ; INSECATEUR
	db ICON_HUMANSHAPE  ; LIPPOUTOU
	db ICON_HUMANSHAPE  ; ELEKTEK
	db ICON_HUMANSHAPE  ; MAGMAR
	db ICON_BUG         ; SCARABRUTE
	db ICON_EQUINE      ; TAUROS
	db ICON_FISH        ; MAGICARPE
	db ICON_LEVIATOR    ; LEVIATOR
	db ICON_LOKHLASS      ; LOKHLASS
	db ICON_BLOB        ; METAMORPH
	db ICON_FOX         ; EVOLI
	db ICON_FOX         ; AQUALI
	db ICON_FOX         ; VOLTALI
	db ICON_FOX         ; PYROLI
	db ICON_VOLTORBE     ; PORYGON
	db ICON_SHELL       ; AMONITA
	db ICON_SHELL       ; AMONISTAR
	db ICON_SHELL       ; KABUTO
	db ICON_SHELL       ; KABUTOPS
	db ICON_BIRD        ; PTERA
	db ICON_RONFLEX     ; RONFLEX
	db ICON_BIRD        ; ARTIKODIN
	db ICON_BIRD        ; ELECTHOR
	db ICON_BIRD        ; SULFURA
	db ICON_SERPENT     ; MINIDRACO
	db ICON_SERPENT     ; DRACO
	db ICON_BIGMON      ; DRACOLOSSE
	db ICON_HUMANSHAPE  ; MEWTWO
	db ICON_HUMANSHAPE  ; MEW
	db ICON_MYSTHERBE      ; GERMIGNON
	db ICON_MYSTHERBE      ; MACRONIUM
	db ICON_MYSTHERBE      ; MEGANIUM
	db ICON_FOX         ; HERICENDRE
	db ICON_FOX         ; FEURISSON
	db ICON_FOX         ; TYPHLOSION
	db ICON_MONSTER     ; KAIMINUS
	db ICON_MONSTER     ; CROCRODIL
	db ICON_MONSTER     ; ALIGATUEUR
	db ICON_FOX         ; FOUINETTE
	db ICON_FOX         ; FOUINAR
	db ICON_BIRD        ; HOOTHOOT
	db ICON_BIRD        ; NOARFANG
	db ICON_BUG         ; COXY
	db ICON_BUG         ; COXYCLAQUE
	db ICON_BUG         ; MIMIGAL
	db ICON_BUG         ; MIGALOS
	db ICON_BAT         ; NOSTENFER
	db ICON_FISH        ; LOUPIO
	db ICON_FISH        ; LANTURN
	db ICON_PIKACHU     ; PICHU
	db ICON_MELOFEE    ; MELO
	db ICON_RONDOUDOU  ; TOUDOUDOU
	db ICON_MELOFEE    ; TOGEPI
	db ICON_BIRD        ; TOGETIC
	db ICON_BIRD        ; NATU
	db ICON_BIRD        ; XATU
	db ICON_FOX         ; WATTOUAT
	db ICON_MONSTER     ; LAINERGIE
	db ICON_MONSTER     ; PHARAMP
	db ICON_MYSTHERBE      ; JOLIFLOR
	db ICON_RONDOUDOU  ; MARILL
	db ICON_RONDOUDOU  ; AZUMARILL
	db ICON_SIMULARBRE   ; SIMULARBRE
	db ICON_PTITARD     ; TARPAUD
	db ICON_MYSTHERBE      ; GRANIVOL
	db ICON_MYSTHERBE      ; FLORAVOL
	db ICON_MYSTHERBE      ; COTOVOL
	db ICON_MONSTER     ; CAPUMAIN
	db ICON_MYSTHERBE      ; TOURNEGRIN
	db ICON_MYSTHERBE      ; HELIATRONC
	db ICON_BUG         ; YANMA
	db ICON_MONSTER     ; AXOLOTO
	db ICON_MONSTER     ; MARAISTE
	db ICON_FOX         ; MENTALI
	db ICON_FOX         ; NOCTALI
	db ICON_BIRD        ; CORNEBRE
	db ICON_RAMOLOSS    ; ROIGADA
	db ICON_GHOST       ; FEUFOREVE
	db ICON_UNOWN       ; UNOWN
	db ICON_GHOST       ; QULBUTOKE
	db ICON_EQUINE      ; GIRAFARIG
	db ICON_BUG         ; POMDEPIK
	db ICON_BUG         ; FORETRESS
	db ICON_SERPENT     ; INSOLOURDO
	db ICON_BUG         ; SCORPLANE
	db ICON_SERPENT     ; STEELIX
	db ICON_MONSTER     ; SNUBBULL
	db ICON_MONSTER     ; GRANBULL
	db ICON_FISH        ; QWILFISH
	db ICON_BUG         ; CIZAYOX
	db ICON_BUG         ; CARATROC
	db ICON_BUG         ; SCARHINO
	db ICON_FOX         ; FARFURET
	db ICON_MONSTER     ; TEDDIURSA
	db ICON_MONSTER     ; URSARING
	db ICON_BLOB        ; LIMAGMA
	db ICON_BLOB        ; VOLCAROPOD
	db ICON_EQUINE      ; MARCACRIN
	db ICON_EQUINE      ; COCHIGNON
	db ICON_SHELL       ; CORAYON
	db ICON_FISH        ; REMORAID
	db ICON_FISH        ; OCTILLERY
	db ICON_MONSTER     ; CADOIZO
	db ICON_FISH        ; DEMANTA
	db ICON_BIRD        ; AIRMURE
	db ICON_FOX         ; MALOSSE
	db ICON_FOX         ; DEMOLOSSE
	db ICON_BIGMON      ; HYPOROI
	db ICON_EQUINE      ; PHANPY
	db ICON_EQUINE      ; DONPHAN
	db ICON_VOLTORBE     ; PORYGON2
	db ICON_EQUINE      ; CERFROUSSE
	db ICON_MONSTER     ; QUEULORIOR
	db ICON_FIGHTER     ; DEBUGANT
	db ICON_FIGHTER     ; KAPOERA
	db ICON_HUMANSHAPE  ; LIPPOUTI
	db ICON_HUMANSHAPE  ; ELEKID
	db ICON_HUMANSHAPE  ; MAGBY
	db ICON_EQUINE      ; ECREMEUH
	db ICON_MELOFEE    ; LEUPHORIE
	db ICON_FOX         ; RAIKOU
	db ICON_FOX         ; ENTEI
	db ICON_FOX         ; SUICUNE
	db ICON_MONSTER     ; EMBRYLEX
	db ICON_MONSTER     ; YMPHECT
	db ICON_MONSTER     ; TYRANOCIF
	db ICON_LUGIA       ; LUGIA
	db ICON_HO_OH       ; HO_OH
	db ICON_HUMANSHAPE  ; CELEBI
	assert_table_length NUM_POKEMON
