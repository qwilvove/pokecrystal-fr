# Delete these rules if you don't care about matching.

%.lz: LZFLAGS = --matching

gfx/battle_anims/%.lz: LZFLAGS += --odd-alternate --align 4

gfx/battle/dude.2bpp.lz: LZFLAGS += --align 4

gfx/card_flip/card_flip_1.2bpp.lz: LZFLAGS += --iterate-only --align 1

gfx/diploma/diploma.2bpp.lz: LZFLAGS += --literal-only --align 1

gfx/memory_game/memory_game.2bpp.lz: LZFLAGS += --odd-alternate --align 4

gfx/intro/%.lz: LZFLAGS += --align 4
gfx/intro/crystal_zarbis.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/intro/suicune_close.2bpp.lz: LZFLAGS += --odd-alternate --align 4
gfx/intro/zarbi_back.2bpp.lz: LZFLAGS += --odd-alternate --align 4

gfx/mobile/mobile_trade.%.lz: LZFLAGS += --align 4
gfx/mobile/mobile_trade_sprites.2bpp.lz: LZFLAGS += --align 4

gfx/new_game/shrink1.2bpp.lz: LZFLAGS += --align 4
gfx/new_game/shrink2.2bpp.lz: LZFLAGS += --align 4

gfx/pc/pc.2bpp.lz: LZFLAGS += --literal-only --align 1

gfx/player/chris_back.2bpp.lz: LZFLAGS += --align 4

gfx/pokedex/pokedex.2bpp.lz: LZFLAGS += --prefer-alternate --align 1
gfx/pokedex/pokedex_sgb.2bpp.lz: LZFLAGS += --prefer-alternate --align 1
gfx/pokedex/question_mark.2bpp.lz: LZFLAGS += --align 4
gfx/pokedex/ramoloss.2bpp.lz: LZFLAGS += --literal-only --align 1

gfx/pokegear/pokegear.2bpp.lz: LZFLAGS += --prefer-alternate --align 1 --skip-initial-byte
gfx/pokegear/pokegear_sprites.2bpp.lz: LZFLAGS += --align 4
gfx/pokegear/town_map.2bpp.lz: LZFLAGS += --literal-only --align 1

gfx/pokemon/corayon/front.animated.2bpp.lz: LZFLAGS += --no-lookback-3
gfx/pokemon/kapoera/back.2bpp.lz: LZFLAGS += --no-lookback-3
gfx/pokemon/nidoran_f/front.animated.2bpp.lz: LZFLAGS += --no-lookback-3
gfx/pokemon/colossinge/front.animated.2bpp.lz: LZFLAGS += --no-lookback-3
gfx/pokemon/insecateur/front.animated.2bpp.lz: LZFLAGS += --no-lookback-3
gfx/pokemon/zarbi_g/front.animated.2bpp.lz: LZFLAGS += --no-lookback-3
gfx/pokemon/voltorbe/front.animated.2bpp.lz: LZFLAGS += --no-lookback-3

gfx/sgb/gbc_only.2bpp.lz: LZFLAGS += --literal-only --align 1

gfx/slots/slots_1.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/slots/slots_2.2bpp.lz: LZFLAGS += --align 4
gfx/slots/slots_3.2bpp.lz: LZFLAGS += --align 4

gfx/splash/metamorph.2bpp.lz: LZFLAGS += --odd-alternate --align 4

gfx/tilesets/%.lz: LZFLAGS += --align 4
gfx/tilesets/ptera_word_room.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/tilesets/battle_tower_outside.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/tilesets/beta_word_room.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/tilesets/ho_oh_word_room.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/tilesets/house.2bpp.lz: LZFLAGS += --odd-alternate --align 4
gfx/tilesets/johto.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/tilesets/johto_modern.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/tilesets/kabuto_word_room.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/tilesets/kanto.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/tilesets/mansion.2bpp.lz: LZFLAGS += --odd-alternate --align 4
gfx/tilesets/mart.2bpp.lz: LZFLAGS += --odd-alternate --align 4
gfx/tilesets/amonita_word_room.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/tilesets/players_room.2bpp.lz: LZFLAGS += --no-lookback-3 --align 4
gfx/tilesets/ruins_of_alph.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/tilesets/tower.2bpp.lz: LZFLAGS += --odd-alternate --align 4

gfx/title/crystal.2bpp.lz: LZFLAGS += --align 4
gfx/title/logo.2bpp.lz: LZFLAGS += --literal-only --align 1
gfx/title/suicune.2bpp.lz: LZFLAGS += --align 4

gfx/zarbi_puzzle/%.lz: LZFLAGS += --align 4
gfx/zarbi_puzzle/start_cancel.2bpp.lz: LZFLAGS += --prefer-alternate --align 1
