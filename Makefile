...
pokecrystal.gbc:         RGBFIXFLAGS += -i BYTF -n 0
pokecrystal_debug.gbc:   RGBFIXFLAGS += -i BYTF -n 0
pokecrystal_vc.gbc:      RGBFIXFLAGS += -i BYTF -n 0
...
# Lamantine has an unused tile id in its last frame. The tile itself is missing.
gfx/pokemon/lamantine/frames.asm: gfx/pokemon/lamantine/front.animated.tilemap gfx/pokemon/lamantine/front.dimensions
	tools/pokemon_animation -f $^ > $@
	echo "	db \$$4d" >> $@
...
gfx/pokemon/zarbi_%/back.2bpp: RGBGFXFLAGS += --colors gbc:$(word 2,$^)
gfx/pokemon/zarbi_%/front.2bpp: RGBGFXFLAGS += --colors gbc:$(word 2,$^)
...
gfx/pokemon/piafabec/normal.gbcpal: tools/gbcpal += --reverse
gfx/pokemon/rapasdepic/normal.gbcpal: tools/gbcpal += --reverse
gfx/pokemon/canarticho/normal.gbcpal: tools/gbcpal += --reverse
gfx/pokemon/kicklee/normal.gbcpal: tools/gbcpal += --reverse
gfx/pokemon/insecateur/normal.gbcpal: tools/gbcpal += --reverse
gfx/pokemon/lippoutou/normal.gbcpal: tools/gbcpal += --reverse
