TypeMatchups:
	;  attacker,     defender,     *=
	db NORMAL,       ROCHE,         NOT_VERY_EFFECTIVE
	db NORMAL,       ACIER,        NOT_VERY_EFFECTIVE
	db FEU,         FEU,         NOT_VERY_EFFECTIVE
	db FEU,         EAU,        NOT_VERY_EFFECTIVE
	db FEU,         PLANTE,        SUPER_EFFECTIVE
	db FEU,         GLACE,          SUPER_EFFECTIVE
	db FEU,         INSECTE,          SUPER_EFFECTIVE
	db FEU,         ROCHE,         NOT_VERY_EFFECTIVE
	db FEU,         DRAGON,       NOT_VERY_EFFECTIVE
	db FEU,         ACIER,        SUPER_EFFECTIVE
	db EAU,        FEU,         SUPER_EFFECTIVE
	db EAU,        EAU,        NOT_VERY_EFFECTIVE
	db EAU,        PLANTE,        NOT_VERY_EFFECTIVE
	db EAU,        SOL,       SUPER_EFFECTIVE
	db EAU,        ROCHE,         SUPER_EFFECTIVE
	db EAU,        DRAGON,       NOT_VERY_EFFECTIVE
	db ELECTRIK,     EAU,        SUPER_EFFECTIVE
	db ELECTRIK,     ELECTRIK,     NOT_VERY_EFFECTIVE
	db ELECTRIK,     PLANTE,        NOT_VERY_EFFECTIVE
	db ELECTRIK,     SOL,       NO_EFFECT
	db ELECTRIK,     VOL,       SUPER_EFFECTIVE
	db ELECTRIK,     DRAGON,       NOT_VERY_EFFECTIVE
	db PLANTE,        FEU,         NOT_VERY_EFFECTIVE
	db PLANTE,        EAU,        SUPER_EFFECTIVE
	db PLANTE,        PLANTE,        NOT_VERY_EFFECTIVE
	db PLANTE,        POISON,       NOT_VERY_EFFECTIVE
	db PLANTE,        SOL,       SUPER_EFFECTIVE
	db PLANTE,        VOL,       NOT_VERY_EFFECTIVE
	db PLANTE,        INSECTE,          NOT_VERY_EFFECTIVE
	db PLANTE,        ROCHE,         SUPER_EFFECTIVE
	db PLANTE,        DRAGON,       NOT_VERY_EFFECTIVE
	db PLANTE,        ACIER,        NOT_VERY_EFFECTIVE
	db GLACE,          EAU,        NOT_VERY_EFFECTIVE
	db GLACE,          PLANTE,        SUPER_EFFECTIVE
	db GLACE,          GLACE,          NOT_VERY_EFFECTIVE
	db GLACE,          SOL,       SUPER_EFFECTIVE
	db GLACE,          VOL,       SUPER_EFFECTIVE
	db GLACE,          DRAGON,       SUPER_EFFECTIVE
	db GLACE,          ACIER,        NOT_VERY_EFFECTIVE
	db GLACE,          FEU,         NOT_VERY_EFFECTIVE
	db COMBAT,     NORMAL,       SUPER_EFFECTIVE
	db COMBAT,     GLACE,          SUPER_EFFECTIVE
	db COMBAT,     POISON,       NOT_VERY_EFFECTIVE
	db COMBAT,     VOL,       NOT_VERY_EFFECTIVE
	db COMBAT,     PSY, NOT_VERY_EFFECTIVE
	db COMBAT,     INSECTE,          NOT_VERY_EFFECTIVE
	db COMBAT,     ROCHE,         SUPER_EFFECTIVE
	db COMBAT,     TENEBRES,         SUPER_EFFECTIVE
	db COMBAT,     ACIER,        SUPER_EFFECTIVE
	db POISON,       PLANTE,        SUPER_EFFECTIVE
	db POISON,       POISON,       NOT_VERY_EFFECTIVE
	db POISON,       SOL,       NOT_VERY_EFFECTIVE
	db POISON,       ROCHE,         NOT_VERY_EFFECTIVE
	db POISON,       SPECTRE,        NOT_VERY_EFFECTIVE
	db POISON,       ACIER,        NO_EFFECT
	db SOL,       FEU,         SUPER_EFFECTIVE
	db SOL,       ELECTRIK,     SUPER_EFFECTIVE
	db SOL,       PLANTE,        NOT_VERY_EFFECTIVE
	db SOL,       POISON,       SUPER_EFFECTIVE
	db SOL,       VOL,       NO_EFFECT
	db SOL,       INSECTE,          NOT_VERY_EFFECTIVE
	db SOL,       ROCHE,         SUPER_EFFECTIVE
	db SOL,       ACIER,        SUPER_EFFECTIVE
	db VOL,       ELECTRIK,     NOT_VERY_EFFECTIVE
	db VOL,       PLANTE,        SUPER_EFFECTIVE
	db VOL,       COMBAT,     SUPER_EFFECTIVE
	db VOL,       INSECTE,          SUPER_EFFECTIVE
	db VOL,       ROCHE,         NOT_VERY_EFFECTIVE
	db VOL,       ACIER,        NOT_VERY_EFFECTIVE
	db PSY, COMBAT,     SUPER_EFFECTIVE
	db PSY, POISON,       SUPER_EFFECTIVE
	db PSY, PSY, NOT_VERY_EFFECTIVE
	db PSY, TENEBRES,         NO_EFFECT
	db PSY, ACIER,        NOT_VERY_EFFECTIVE
	db INSECTE,          FEU,         NOT_VERY_EFFECTIVE
	db INSECTE,          PLANTE,        SUPER_EFFECTIVE
	db INSECTE,          COMBAT,     NOT_VERY_EFFECTIVE
	db INSECTE,          POISON,       NOT_VERY_EFFECTIVE
	db INSECTE,          VOL,       NOT_VERY_EFFECTIVE
	db INSECTE,          PSY, SUPER_EFFECTIVE
	db INSECTE,          SPECTRE,        NOT_VERY_EFFECTIVE
	db INSECTE,          TENEBRES,         SUPER_EFFECTIVE
	db INSECTE,          ACIER,        NOT_VERY_EFFECTIVE
	db ROCHE,         FEU,         SUPER_EFFECTIVE
	db ROCHE,         GLACE,          SUPER_EFFECTIVE
	db ROCHE,         COMBAT,     NOT_VERY_EFFECTIVE
	db ROCHE,         SOL,       NOT_VERY_EFFECTIVE
	db ROCHE,         VOL,       SUPER_EFFECTIVE
	db ROCHE,         INSECTE,          SUPER_EFFECTIVE
	db ROCHE,         ACIER,        NOT_VERY_EFFECTIVE
	db SPECTRE,        NORMAL,       NO_EFFECT
	db SPECTRE,        PSY, SUPER_EFFECTIVE
	db SPECTRE,        TENEBRES,         NOT_VERY_EFFECTIVE
	db SPECTRE,        ACIER,        NOT_VERY_EFFECTIVE
	db SPECTRE,        SPECTRE,        SUPER_EFFECTIVE
	db DRAGON,       DRAGON,       SUPER_EFFECTIVE
	db DRAGON,       ACIER,        NOT_VERY_EFFECTIVE
	db TENEBRES,         COMBAT,     NOT_VERY_EFFECTIVE
	db TENEBRES,         PSY, SUPER_EFFECTIVE
	db TENEBRES,         SPECTRE,        SUPER_EFFECTIVE
	db TENEBRES,         TENEBRES,         NOT_VERY_EFFECTIVE
	db TENEBRES,         ACIER,        NOT_VERY_EFFECTIVE
	db ACIER,        FEU,         NOT_VERY_EFFECTIVE
	db ACIER,        EAU,        NOT_VERY_EFFECTIVE
	db ACIER,        ELECTRIK,     NOT_VERY_EFFECTIVE
	db ACIER,        GLACE,          SUPER_EFFECTIVE
	db ACIER,        ROCHE,         SUPER_EFFECTIVE
	db ACIER,        ACIER,        NOT_VERY_EFFECTIVE

	db -2 ; end (with Foresight)

; Foresight removes Spectre's immunities.
	db NORMAL,       SPECTRE,        NO_EFFECT
	db COMBAT,     SPECTRE,        NO_EFFECT

	db -1 ; end
